...
import { FIN_DE_TOUR_DEBUT_MS, FIN_DE_TOUR_MAX_MS, FIN_DE_TOUR_PAS_MS, creerAttendreFinDuTour, type DepsFinDeTour } from '../src/mba/fin-de-tour';
import { DELAI_REPONSE_ENVOI_MS } from '../src/http/mba-relais';
 ...
 */
function faux(o: { echos?: Array<string | null>; leve?: boolean; selonHeure?: (t: number) => string | null } = {}) {
  let t = 0;
  let lectures = 0;
...
describe('attendre la fin du tour de l’agent de Meta', () => {
  it('🔴 s’arrête dès que l’agent a parlé (son dernier écho CHANGE)', async () => {
    ...
    expect(f.attentes).toEqual([FIN_DE_TOUR_DEBUT_MS, FIN_DE_TOUR_PAS_MS, FIN_DE_TOUR_PAS_MS, FIN_DE_TOUR_PAS_MS]);
  ...
  it('🔴 un écho arrivé AVANT la réponse du relais (texte écrit avant l’appel d’outil) n’est pas la fin du tour', async () => {
    // L'agent a écrit « Je regarde ça » avant d'appeler l'outil ; son écho nous arrive à t = 1 000 ms. Relevé à
    // t = 0, il passerait pour sa phrase d'annonce, et le fil serait pris en plein tour (revue du 2026-09-22).
    const f = faux({ selonHeure: (t) => (t < 1000 ? 'ancien' : 'avant-outil') });
    expect(await f.attendre('t1', 'w1')).toBe('delai');
  });

  it('🔴 on commence à guetter quand le relais a répondu : les deux délais restent ÉGAUX', () => {
    expect(FIN_DE_TOUR_DEBUT_MS).toBe(DELAI_REPONSE_ENVOI_MS);
  });
