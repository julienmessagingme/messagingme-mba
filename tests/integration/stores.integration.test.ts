...
/** Un identifiant de campagne qui n'existe pas : sert a prouver qu'un filtre par campagne filtre vraiment. */
const AUTRE_CAMPAGNE = '33333333-3333-4333-8333-333333333333';
...
describe.skipIf(!url)('adaptateurs Postgres (Supabase)', () => {
  ...
  it('PgStatsStore : campaign funnel (répondu=inbound après envoi), error breakdown, contacts touchés, cost volume', async () => {
    ...
    // La ligne porte SA campagne : c'est ce qui permet de filtrer les erreurs par campagne cote ecran.
    expect(errors.find((e) => e.code === 131026)).toMatchObject({ campaignId, campaignName: 'Funnel' });

    // 🔴 QUI a ete touche, et c'est le JOURNAL DES ERREURS DE LIVRAISON qui repond, pas une seconde requete
    // ecrite pour Analytics. L'invariant qui compte est que la liste ait EXACTEMENT autant de lignes que le
  ...
  // Forme COMPLÈTE de TenantSettings : évite les toEqual qui périment quand un réglage est ajouté.
  ...
  // ⚠️ LE COMPTE N'EST PLUS ÉCRIT ICI : cette ligne a annoncé « 9 clés » pendant qu'il y en avait HUIT, et
  // l'ajout d'`optoutRequestId` le 2026-09-13 l'a rendue vraie par accident. Un nombre à la main ne tient pas.
