...
import { manquesDeCouverture, DIMENSIONS } from '../src/agent/setup/couverture';
...
describe('couverture du périmètre', () => {
  it('rien de couvert : les six points manquent', () => {
    expect(manquesDeCouverture([])).toEqual(DIMENSIONS.map((d) => d.code));
  });

  it('tout couvert : plus rien ne manque', () => {
    expect(manquesDeCouverture(DIMENSIONS.map((d) => d.code))).toEqual([]);
  });

  it('🔴 un code INVENTÉ ne couvre rien', () => {
    // La couverture vient d'un modèle, donc d'une source non fiable : s'il suffisait d'inventer six codes
    // pour la déclarer complète, l'entretien ne serait qu'une suggestion.
    expect(manquesDeCouverture(['mission', 'tout_le_reste', 'perimetre_'])).toEqual(
      DIMENSIONS.map((d) => d.code).filter((c) => c !== 'mission'),
    );
  });

  it('un doublon ne compte qu’une fois', () => {
    expect(manquesDeCouverture(['ton', 'ton', 'ton'])).not.toContain('ton');
  });
});
