...
describe('accusés Meta -> mesure par bloc', () => {
  it('🔴 délivré et lu sont rattachés, avec leur identifiant de message', async () => {
    ...
    await processStatuses([statut('wamid.1', 'delivered'), statut('wamid.1', 'read')], store as never, { tarifs: aucunTarif, echecsLibres: aucunEchecLibre, nodeEvents: sink });
  ...
  it('🔴 le statut « sent » n’est PAS rattaché : il doublerait chaque envoi', async () => {
    ...
    const { recus, sink } = puits();
    await processStatuses([statut('wamid.1', 'sent')], store as never, { tarifs: aucunTarif, echecsLibres: aucunEchecLibre, nodeEvents: sink });
  ...
  it('un échec est rattaché aussi (un bloc qui échoue est une mesure en soi)', async () => {
    const { store } = livraison();
    const { recus, sink } = puits();
    await processStatuses([statut('wamid.2', 'failed', 131026)], store as never, { tarifs: aucunTarif, echecsLibres: aucunEchecLibre, nodeEvents: sink });
  ...
  it('🔴 une mesure en échec n’empêche PAS la mise à jour de la livraison', async () => {
    ...
    await processStatuses([statut('wamid.3', 'read')], store as never, { tarifs: aucunTarif, echecsLibres: aucunEchecLibre, nodeEvents: sink });
    expect(majs).toEqual([{ id: 'wamid.3', status: 'read' }]);
  });

  it('sans puits de mesure, le comportement est exactement celui d’avant', async () => {
    const { majs, store } = livraison();
    await processStatuses([statut('wamid.4', 'delivered')], store as never, { tarifs: aucunTarif, echecsLibres: aucunEchecLibre });
