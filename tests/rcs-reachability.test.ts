...
class CountingProvider implements RcsProvider {
  ...
  async send(_t: string, _a: string, _e: string, _m: RcsOutbound, messageId: string): Promise<SendResult> {
    return { messageId };
  }
}
...
describe('Reachability', () => {
  it('interroge le provider une seule fois puis sert le cache', async () => {
    ...
    expect(await r.isReachable('t1', 'agent-1', '+33600000002')).toBe(true);
    expect(await r.isReachable('t1', 'agent-1', '+33600000002')).toBe(true);
  ...
  it('reinterroge le provider quand l entree depasse le TTL', async () => {
    ...
    await r.isReachable('t1', 'agent-1', '+33600000002');
    now = 1_000 + TTL_MS + 1;
    await r.isReachable('t1', 'agent-1', '+33600000002');
  ...
  it('met un NON joignable en cache aussi', async () => {
    ...
    expect(await r.isReachable('t1', 'agent-1', '+33600000001')).toBe(false);
  ...
  it('propage l erreur du provider quand il n y a AUCUNE entree en cache', async () => {
    ...
    await expect(r.isReachable('t1', 'agent-1', '+33600000002')).rejects.toThrow('provider indisponible');
  });
});
