...
describe('FakeRcsProvider', () => {
  it('renvoie null pour un numero declare non joignable', async () => {
    ...
    expect(await p.capabilities('t1', 'agent-1', '+33600000001')).toBeNull();
    expect(await p.capabilities('t1', 'agent-1', '+33600000002')).not.toBeNull();
  });

  it('journalise chaque envoi avec son messageId et le rend tel quel', async () => {
    const p = new FakeRcsProvider();
    const r = await p.send('t1', 'agent-1', '+33600000002', { kind: 'text', text: 'Bonjour' }, 'msg-42');
  ...
  it('ignore un messageId deja utilise pour le meme agent (idempotence RBM)', async () => {
    const p = new FakeRcsProvider();
    await p.send('t1', 'agent-1', '+33600000002', { kind: 'text', text: 'A' }, 'msg-1');
    await p.send('t1', 'agent-1', '+33600000002', { kind: 'text', text: 'B' }, 'msg-1');
  ...
  it('ne confond PAS deux agents qui utilisent le meme messageId', async () => {
    const p = new FakeRcsProvider();
    await p.send('t1', 'agent-1', '+33600000002', { kind: 'text', text: 'A' }, 'msg-1');
    await p.send('t1', 'agent-2', '+33600000002', { kind: 'text', text: 'B' }, 'msg-1');
