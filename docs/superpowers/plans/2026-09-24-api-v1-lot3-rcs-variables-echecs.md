...
- **`web/lib/qui-a-repondu.ts`** (commentaire de l'origine `api`, que le lot 2 a laissé à ce lot) affirme qu'un message de l'API « ne peut arriver qu'APRÈS que la personne a écrit », pour justifier le badge « scripté ». C'est faux dès `POST /v1/messages/rcs` (aucune fenêtre, une fiche qui a seulement consenti suffit) : 🔴 **TRANCHÉ PAR JULIEN LE 2026-09-24** : un message de l'API ne compte comme « Scripté » dans la colonne « Répondu par » que s'il RÉPOND à un client qui avait déjà écrit dans ce fil ; quand il écrit LE PREMIER, il ne compte pas, exactement comme un envoi de campagne. Ce lot le code : la lecture qui fabrique les `origines` d'une conversation (l'appelant de `repondeursDe`) ne garde un sortant d'origine `api` que si un entrant du même fil le PRÉCÈDE ; `repondeursDe` et son test gagnent le cas (un `api` seul ne donne rien, un `api` après un entrant donne « Scripté ») ; le commentaire est réécrit avec la vraie raison. Test vérifié dans les deux sens. Tout cela se fait dans ce lot (aucun autre plan ne touche ce fichier : le lot 4 ne réécrit que la page de doc et `features.md`). Le commit de cette correction ajoute `web/lib/qui-a-repondu\.ts` au motif du contrôle d'intrus du gabarit de commit.
