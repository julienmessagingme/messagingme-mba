...
| ~~D6~~ | ~~**La migration 0086 n'est pas appliquée en production**~~ **FERMÉE le 2026-08-28** : appliquée sur la base de prod (`0086_agent_ia.sql ... ok`), les cinq tables `agent*` vérifiées présentes, puis les trois conteneurs déployés sur `a325844`. Séquence respectée : `build mba-api` AVANT `migrate` (les migrations vivent dans l'image), `up -d --build` après. | Fermée. | Sans objet. |
