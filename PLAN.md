...
**État au 2026-07-24** : blocs 0, 2, **3** et **1** livrés ET DÉPLOYÉS ; bloc **4 en cours (4/13)** ; bloc A aux 3/5.

**TOUT est en production et vérifié** (mba VPS = commit `48a5ace`, migrations 0042/0043/0044 appliquées ; mm-hubspot VPS = `526306b`, dépôt GitHub privé). Détail :
...
- **Bloc 4** en cours (décision Julien 2026-07-24 : TOUT le bloc 4, dans l'ordre par valeur, Railway ACTÉ) :
  ...
  - **RESTE 9 items** : 4.10 (sweeper qualité numéros, **PROCHAIN**), 4.3 (mode transaction), 4.7 (health), 4.11 (CA Supabase), 4.12 (tsx deps), 4.4 (BACKEND_URL), 4.13 (restore), 4.2 (bind IPv6, Railway), 4.8 (advisory lock, Railway). Plans détaillés + vérifs adversariales : `.loop/bloc4.md` + `scratchpad/bloc4-plans.json` / `bloc4-verifs.json`.
**Prochain : bloc 4 à 4.10.** Puis A.3/A.5 (attendent MBA), puis 5. ⚠️ Dernière migration = 0044, prochaine = 0045.
