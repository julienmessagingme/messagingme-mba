...
/**
 * E2E builder : le bloc « Action » s'ajoute (par défaut « ajouter un tag »), on change la sous-action en « retirer un
 * tag », on saisit un tag, et l'auto-save persiste un node `type:'action'` avec `actionKind:'remove_tag'` + tag.
...
test.describe('Builder : node Action', () => {
  test('ajoute un bloc Action, choisit « retirer un tag », et l’auto-save persiste type:action + actionKind', async ({ page }) => {
    ...
    await expect.poll(
      () => saved.some((g) => g.nodes.some((nd) => nd.type === 'action' && (nd.data as { actionKind?: unknown }).actionKind === 'remove_tag' && (nd.data as { tag?: unknown }).tag === 'vip')),
      { timeout: 10_000 },
    ).toBe(true);
  });
});
