    ...
    <div className="flex flex-col gap-2">
      ...
      {brouillon.variables.map((v, i) => (
        <div key={i} className="flex flex-wrap items-start gap-2">
          ...
          {v.origine.type === 'fixe' && (
            <input
              className={`${inputCls} w-56`} data-testid={`var-fixe-${i}`} value={String(v.origine.valeur)}
          ...
          <select className={`${inputCls} w-28`} aria-label={t('Type', 'Type')} data-testid={`var-type-${i}`} value={v.type} onChange={(e) => changer(i, { type: e.target.value as VariableRequete['type'] })}>
