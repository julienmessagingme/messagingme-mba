...
function FieldValueEditor({ d, fields, onPatch, avecValeur }: {
  ...
  return (
    <div className="space-y-2">
      <div>
        <label className="mb-1 block text-xs font-medium text-ink-500">{t('Champ', 'Field')}</label>
        <select
      ...
      {avecValeur && (
        <div>
          <label className="mb-1 block text-xs font-medium text-ink-500">{t('Valeur', 'Value')}</label>
          <select
...
export function ConfigPanel({
  ...
  return (
    <div className="space-y-3">
      ...
      <div>
        <label className="mb-1 block text-xs font-medium text-ink-500">{t('Nom du bloc', 'Block name')}</label>
      ...
      {wfType === 'wait' && (() => {
        ...
        return (
        <div>
          <label className="mb-1 block text-xs font-medium text-ink-500">{t('Reprendre le parcours…', 'Resume the parcours…')}</label>
        ...
        return (
          <div className="space-y-3">
            ...
            <div className={composeIci ? 'hidden' : undefined}>
              <label className="mb-1 block text-xs font-medium text-ink-500">{t('Partir d’un message enregistré', 'Start from a saved message')}</label>
              <select
                value=""
                onChange={(e) => {
            ...
            <div className={composeIci ? undefined : 'hidden'}>
              <label className="mb-1 block text-xs font-medium text-ink-500">{t('Image d’en-tête (facultatif)', 'Header image (optional)')}</label>
              <ChampImageHebergee
                tenantId={tenantId}
            ...
            <div className={composeIci ? undefined : 'hidden'}>
              <label className="mb-1 block text-xs font-medium text-ink-500">{t('Boutons', 'Buttons')}</label>
              <RcsButtonsEditor
                boutons={boutons}
      ...
      {wfType === 'template' && (
        <div>
          <label className="mb-1 block text-xs font-medium text-ink-500">{t('Template à envoyer', 'Template to send')}</label>
      ...
      {wfType === 'question' && (() => {
        ...
        return (
          <div className="space-y-3">
            ...
            <div>
              <label className="mb-1 block text-xs font-medium text-ink-500">{t('Bouton qui ouvre le menu', 'Button that opens the menu')}</label>
              <input
            ...
            <div>
              <label className="mb-1 block text-xs font-medium text-ink-500">{t('Si le contact ne répond pas', 'If the contact does not reply')}</label>
      ...
      {wfType === 'quick_message' && (() => {
        ...
        return (
          <div className="space-y-2">
            ...
            <div>
              <label className="mb-1 block text-xs font-medium text-ink-500">{t('Image (facultatif)', 'Image (optional)')}</label>
              <ChampImageHebergee
                tenantId={tenantId}
                valeur={(d.imageUrl as string) ?? ''}
                onChange={(imageUrl) => onPatch({ imageUrl })}
            ...
            <div className="rounded-lg border border-ink-200 bg-ink-50/50 p-2">
              ...
              {lienActif && (
                <div className="mt-2 space-y-1.5">
                  ...
                  />
                  {lienMauvaiseUrl && (
                    <p className="text-xs text-danger" data-testid="quick-node-lien-erreur">
      ...
      {wfType === 'flow' && (
        <div className="space-y-2">
          <div>
            <label className="mb-1 block text-xs font-medium text-ink-500">{t('Formulaire (publié)', 'Form (published)')}</label>
          ...
          <div>
            <label className="mb-1 block text-xs font-medium text-ink-500">{t('Libellé du bouton', 'Button label')}</label>
      ...
      {wfType === 'action' && (() => {
        ...
        return (
          <div className="space-y-2">
            <div>
              <label className="mb-1 block text-xs font-medium text-ink-500">{t('Action', 'Action')}</label>
      ...
      {wfType === 'http' && (
        <div className="flex flex-col gap-2">
          ...
          <label className="text-xs font-medium text-ink-900">{t('Quel appel', 'Which call')}</label>
          <select
      ...
      {wfType === 'agent' && (
        <div className="flex flex-col gap-2">
          <label className="text-xs font-medium text-ink-900">{t('Quel agent tient la conversation', 'Which agent holds the conversation')}</label>
          <select
      ...
      {wfType === 'email' && (() => {
        ...
        return (
          <div className="space-y-2">
            <div>
              <label className="mb-1 block text-xs font-medium text-ink-500">{t('Boîte d’envoi', 'Sending mailbox')}</label>
              <select
            ...
            <div>
              <label className="mb-1 block text-xs font-medium text-ink-500">{t('Modèle', 'Template')}</label>
              <select
...
function FonctionJs({ tenantId, code, champSource, champCible, fields, onPatch, onCreerChamp }: {
  ...
  return (
    <div className="flex flex-col gap-2">
      ...
      <label className="text-xs font-medium text-ink-900">{t('Champ à transformer', 'Field to transform')}</label>
      <select
      ...
      <label className="text-xs font-medium text-ink-900">{t('Votre transformation', 'Your transformation')}</label>
      {/**
      ...
      */}
      ...
      <label className="text-xs font-medium text-ink-900">{t('Essayer avec cette valeur', 'Try with this value')}</label>
      <div className="flex gap-2">
        <input
        ...
        />
        <Bouton variante="secondaire" taille="petite" enCours={enCours}
      ...
      <label className="text-xs font-medium text-ink-900">{t('Où ranger le résultat', 'Where to store the result')}</label>
      <div className="flex gap-2">
        <select
      ...
      {onCreerChamp && nouveauChamp !== null && (
        <div className="flex flex-col gap-1">
          <div className="flex gap-2">
            ...
            />
            <Bouton variante="secondaire" taille="petite" enCours={creationEnVol}
