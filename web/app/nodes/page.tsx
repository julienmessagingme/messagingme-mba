    ...
    <div className="max-w-liste space-y-6">
        ...
        {[...NODE_ORDER, ...RCS_NODE_ORDER, ...EMAIL_NODE_ORDER, ...AGENT_NODE_ORDER].map((type) => {
          ...
          return (
            <button key={type} onClick={() => setFilter(type)} className={chip(filter === type)}>
              <Icone nom={meta.icone} taille="petite" className="mr-1 inline align-[-0.15em]" />{t(meta.label[0], meta.label[1])}
            </button>
          );
        })}
      </div>
      ...
      <div className="overflow-hidden rounded-carte border border-ink-200 bg-white">
        <div className="border-b border-ink-100 px-5 py-3 text-sm font-semibold text-ink-900">
          {filter === 'all' ? t('Tous les blocs', 'All blocks') : <><Icone nom={NODE_META[filter].icone} className="mr-1.5 inline align-[-0.15em]" />{t(NODE_META[filter].label[0], NODE_META[filter].label[1])}</>}
          <span className="ml-2 text-xs font-normal text-ink-500">({visible.length})</span>
        </div>
        {loading ? (
              ...
              {visible.map((n, i) => {
                ...
                return (
                  <tr key={`${n.workflowId}-${n.code ?? i}`} className="border-b border-ink-50 last:border-0">
                    <td className="whitespace-nowrap px-5 py-3 text-ink-900"><Icone nom={meta.icone} className="mr-1.5 inline align-[-0.15em]" />{t(meta.label[0], meta.label[1])}</td>
                    {/* Repli le temps que les blocs soient renommés : nom libre, sinon le résumé auto, sinon (sans nom). */}
                    <td className="px-5 py-3 text-ink-500">{n.name || n.summary || <span className="text-ink-500">{t('(sans nom)', '(unnamed)')}</span>}</td>
                    ...
                    <td className="px-5 py-3">
                      {n.code
                        ? <code className="rounded-controle bg-ink-100 px-1.5 py-0.5 font-mono text-xs text-ink-500">{n.code}</code>
                        : <span className="text-xs text-ink-500" title={t('Code généré au prochain enregistrement du scénario', 'Code generated on the next scenario save')}>{t('non codé', 'not coded')}</span>}
                    </td>
                  </tr>
                );
              })}
