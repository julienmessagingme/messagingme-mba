...
- ⏳ **CE QUI MANQUE POUR LE LOT 3** : les points 4 et 5 de la spec § 11, côté Julien. Lier la Page au
  numéro WhatsApp (le code arrive dans l'Inbox) et poser un moyen de paiement. ⚠️ Et la Page à choisir
  est CELLE QUI SERA LIÉE au numéro : c'est elle qui décide vers quel WhatsApp le bouton ouvre la
  conversation.
- ⚠️ **MessagingMe ne peut pas connecter SON PROPRE compte par cet écran**, et c'est structurel chez
  Meta : le portefeuille qui possède l'app ne peut pas être son propre client. Pour nos pubs à nous, il
  faudra un utilisateur système créé à la main et déposé par `/ops`. ⚠️ L'App Review, elle, n'est PAS
  bloquée : la démonstration se fait avec un portefeuille client, ce qui est le cas d'usage réel.
